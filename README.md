...
По умолчанию глоссарий работает через промт-инструкции и удаляет только скобочные дубли типа `JWT (GVT)`. Если LLM всё равно использует некорректные варианты в тексте, можно включить агрессивную постобработку:

```bash
uv run main.py path\to\transcript.txt --force-replace-variants
```

Этот режим заменяет **все** отдельные слова, совпадающие с вариантами из глоссария (без учёта регистра), на канонические термины. Например:
- `GVT` → `JWT`
- `кавка` → `Kafka`
- `Кавка` → `Kafka`

**Внимание**: используйте этот режим осторожно — он может заменить варианты даже в контекстах, где это нежелательно (например, в прямых цитатах или именах файлов).
...
uv run main.py path\to\transcript.txt --force-replace-variants --no-glossary-cleanup
```
